...
figure_2 <- ggplot(filter(df_n_change_year,hcovany=='with insurance'), aes(x=year,y=pct_of_total)) +
  ...
  scale_x_continuous(breaks= pretty_breaks()) +
  xlab('Year') +
  ylab('Percent of sample with any insurance') +
...
figure_4 <- ggplot(data = df_pct_share_breakdown) + 
  ...
  scale_x_continuous(breaks= pretty_breaks()) +
  xlab('Year') +
  ylab('Percent of share of those with any insurance') +
...
figure_8 <- ggplot(df_race_white, aes(year, frac_covered)) + 
  geom_line() +
  xlab('Year') +
  ylab('Percent of white individuals') +
...
figure_9 <- ggplot(df_race_nonwhite, aes(year, frac_covered)) + 
  geom_line() +
  xlab('Year') +
  ylab('Percent of nonwhite individuals') +
...
figure_10 <- plot_usmap(data = filter(df_state_change, year == 2008), values = 'pct_tot') +
  scale_fill_continuous(name = 'Percentage',low = 'white',high='dark orange') +
  theme(legend.position = "right") +
  ggtitle('Figure 10. Share of sample with any insurance: 2008')
...
figure_11 <- plot_usmap(data = filter(df_state_change, year == 2018), values = 'pct_tot') +
  scale_fill_continuous(name = 'Percentage',low = 'white',high='dark orange') +
  theme(legend.position = "right") +
  ggtitle('Figure 11. Share of sample with any insurance: 2018')
...
figure_12 <- plot_usmap(data = filter(df_state_change, year == 2018), values = 'pct_diff') +
  scale_fill_continuous(low = 'white', high = 'navy', name = 'Percentage-point change') +
  theme(legend.position = "right") +
...
figure_13 <- ggplot(filter(df_state_change, year == 2008), aes(x = pct_tot, y = pct_diff)) +
  ...
  ylab('Percentage-point change from 2008 to 2018') +
  scale_x_continuous(breaks= pretty_breaks()) +
...
lm_fe_1 <- plm(pct_covered_any_pct_pt_change ~ lag_pct_covered_any + pct_medicaid_of_any + pct_medicare_of_any + 
                 pct_inc_below_20k + lag_pct_inc_below_20k + lag_avg_inc + pct_married_simplified + pct_non_white + avg_wt,
                           data = pdata_lm_fe, model = "within",effect = "twoways")

table_1 <- stargazer(lm_fe_1, header = FALSE, type = 'latex')
